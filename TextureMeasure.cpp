...
const char *pShader = R"(
    ...
    void main() {
        ...
        vec3 col = useTexture == 1? texture(textureImage, vUV).rgb : color;
...
void Display(GLFWwindow* w) {
	...
	SetUniform(program, "useTexture", showTexture? 1 : 0);
...
/* from www.geometry.caltech.edu/pubs/MHBD02.pdf
// Generalized Barycentric Coordinates on Irregular Polygons, by Meyer, Lee, Barr, Desbrun
// compute the cotangent of the non-degenerate triangle abc at vertex b
float Cotangent(vec2 a, vec2 b, vec2 c) {
	vec2 ba = a-b, bc = c-b;
...
void ComputeBarycentric(vec2 p, vector<int> &Q, vector<float> &weights) {
	double weightSum = 0;
	int n = (int) Q.size();
	weights.resize(n);
	for (int i = 0; i < n; i++) {
		vec2 q = uvs[Q[i]], prev = uvs[Q[(i+n-1)%n]], next = uvs[Q[(i+1)%n]];
		weights[i] = (Cotangent(p, q, prev)+Cotangent(p, q, next))/length(q-p);
	}
} */
